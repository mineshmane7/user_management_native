...
const DashboardScreen = ({route, navigation}) => {
  ...
  const [newRolePermissions, setNewRolePermissions] = useState({ addProperty: false, updateProperty: false, deleteProperty: false });
      ...
      if (result && result[0]) {
        ...
        try {
          ...
          try {
            ...
            console.log('Fetch succeeded, content length:', fileContent ? fileContent.length : 0);
          } catch (fetchError) {
  ...
  const renderUserItem = ({item}) => {
    ...
    if (Array.isArray(item.roles)) {
      rolesDisplay = item.roles.map(roleId => getRoleDisplayName(roleId)).join(', ');
    } else if (item.role) {
    ...
    return (
      <View style={styles.itemCard}>
        <View style={styles.itemContent}>
          ...
          <Text style={styles.itemMeta}>Created by: {item.createdByName || 'Unknown'}</Text>
        </View>
        ...
        {isSuperAdmin() && (
          <>
            <View style={{flexDirection: 'row', justifyContent: 'center', marginTop: 10}}>
              <TouchableOpacity
                style={[styles.tabButton, {backgroundColor: !rolesTab ? '#34C759' : '#ddd', marginRight: 10}]}
                onPress={() => setRolesTab(false)}>
                <Text style={{color: !rolesTab ? '#fff' : '#333'}}>User Management</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.tabButton, {backgroundColor: rolesTab ? '#34C759' : '#ddd'}]}
                onPress={() => setRolesTab(true)}>
                <Text style={{color: rolesTab ? '#fff' : '#333'}}>Roles Management</Text>
              </TouchableOpacity>
            </View>
            ...
            {!rolesTab ? (
              ...
              <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setNewRoleName('');
                      setNewRoleId('');
                      setNewRolePermissions({ addProperty: false, updateProperty: false, deleteProperty: false });
                ...
                <FlatList
                  ...
                  renderItem={({item}) => (
                    <View style={styles.itemCard}>
                      <View style={styles.itemContent}>
                        ...
                        <Text style={styles.itemMeta}>Permissions: {Object.entries(item.permissions).map(([k,v]) => v ? k : null).filter(Boolean).join(', ') || 'None'}</Text>
                      </View>
                      <View style={styles.itemActions}>
                        <TouchableOpacity
                          style={[styles.actionButton, styles.editButton]}
                  ...
                  scrollEnabled={false}
                  ListEmptyComponent={<Text style={styles.emptyText}>No roles found</Text>}
                />
              </View>
            )}
          </>
      ...
      <Modal
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.optionsText}>Choose how you want to add users:</Text>
            <TouchableOpacity
              ...
              <Text style={styles.optionButtonSubtext}>Create one user at a time</Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              <Text style={styles.optionButtonSubtext}>Bulk import multiple users</Text>
            </TouchableOpacity>
            <TouchableOpacity
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.rolesContainer}>
              <Text style={styles.pickerLabel}>Roles (select one or more):</Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput
              ...
              autoCapitalize="none"
            />
            <Text style={{fontWeight:'600',marginBottom:5}}>Permissions:</Text>
            <View style={{flexDirection:'row',marginBottom:10}}>
              <TouchableOpacity
                style={[styles.tabButton, {backgroundColor: newRolePermissions.addProperty ? '#34C759' : '#ddd', marginRight: 5}]}
                onPress={() => setNewRolePermissions(p => ({...p, addProperty: !p.addProperty}))}>
                <Text style={{color: newRolePermissions.addProperty ? '#fff' : '#333'}}>Add</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.tabButton, {backgroundColor: newRolePermissions.updateProperty ? '#34C759' : '#ddd', marginRight: 5}]}
                onPress={() => setNewRolePermissions(p => ({...p, updateProperty: !p.updateProperty}))}>
                <Text style={{color: newRolePermissions.updateProperty ? '#fff' : '#333'}}>Update</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.tabButton, {backgroundColor: newRolePermissions.deleteProperty ? '#34C759' : '#ddd'}]}
                onPress={() => setNewRolePermissions(p => ({...p, deleteProperty: !p.deleteProperty}))}>
                <Text style={{color: newRolePermissions.deleteProperty ? '#fff' : '#333'}}>Delete</Text>
              </TouchableOpacity>
            </View>
            <View style={styles.modalButtons}>
              <TouchableOpacity
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity
              ...
              <Text style={styles.buttonText}>📁 Pick CSV File from Device</Text>
            </TouchableOpacity>
